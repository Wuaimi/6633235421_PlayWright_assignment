...
test("Transfer fail (account invalid)", async ({ page }) => {
  await page.goto("https://atm-buddy-lite.lovable.app/", {
    waitUntil: "networkidle",
  });
  ...
  //input another account's number (non existed)
  await page.getByPlaceholder("กรอกหมายเลขบัญชี 6 หลัก").fill("111111");
  ...
  const transferToast = page.locator('li[role="status"][data-state="open"]').last();
  await expect(transferToast).toBeVisible({ timeout: 10000 });
  await expect(transferToast).toContainText("ไม่พบบัญชีปลายทาง");
});

test("Transfer fail (amount invalid)", async ({ page }) => {
  await page.goto("https://atm-buddy-lite.lovable.app/", {
    waitUntil: "networkidle",
  });
  ...
  await page.getByPlaceholder("0").fill("1000000");
   ...
   const transferBtn = page.getByRole("button", { name: /โอนเงิน/ });
